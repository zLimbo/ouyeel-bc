...
//        service = CITAj.build(new HttpService("https://" + InvoiceControl.HOST + ":1337"));
...
//        hashMap.put("chainName", chainName);
        hashMap.put("chainName", "ouyeel_chain");
        hashMap.put("genesisTS", genesisTS);
        hashMap.put("genesisTS", new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(Long.valueOf(genesisTS)));
        ...
        hashMap.put("headerTimestamp", new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(header.getTimestamp()));
        ...
        txAllNumber = InvoiceData.curPos + InvoiceData.ITEM_SIZE;
        if (txAllNumber > InvoiceData.invoicesBuffer.size()) {
            txAllNumber = InvoiceData.invoicesBuffer.size();
        }
